...
class LangMasterController extends Controller
{
    ...
    public function list(Request $request)
    {
        ...
        return view('admin.system.language_master.list');
    }

    public function get_data(Datatables $datatables, Request $request)
    {
        $query = SysLanguageMaster::where('status', 1);
        ...
        return $datatables->eloquent($query)
            ...
            ->addColumn('action', function ($data) {
                return '<a href="' . route('admin.langmaster.edit', $data->id) . '" class="btn btn-xs btn-primary" title="' . ucwords(lang('edit', $this->translation)) . '"><i class="fa fa-pencil"></i>&nbsp; ' . ucwords(lang('edit', $this->translation)) . '</a>';
            })
    ...
    public function create()
    {
        ...
        return view('admin.system.language_master.form');
    }
    ...
    public function do_create(Request $request)
    {
        ...
        // LARAVEL VALIDATION
        $validation = [
            'phrase' => 'required|unique:sys_language_master,phrase'
        ];
        $message = [
        ...
        $names = [
            'phrase' => ucwords(lang('phrase', $this->translation))
        ];
        $this->validate($request, $validation, $message, $names);

        // HELPER VALIDATION FOR PREVENT SQL INJECTION & XSS ATTACK
        $phrase = Helper::validate_input_text($request->phrase);
        if (!$phrase) {
            return back()
                ->withInput()
                ->with('error', lang('Invalid format for #item', $this->translation, ['#item' => ucwords(lang('phrase', $this->translation))]));
        }
        $status = (int) $request->status;

        // SAVE THE DATA
        $data = new SysLanguageMaster();
        $data->phrase = $phrase;
        $data->status = $status;

        if ($data->save()) {
            // SET REDIRECT URL
            $redirect_url = 'admin.langmaster.list';
            if (Helper::validate_input_text($request->input_again) == 'yes') {
                $redirect_url = 'admin.langmaster.create';
            }
            // SUCCESS
            return redirect()
                ->route($redirect_url)
                ->with('success', lang('Successfully added a new #item : #name', $this->translation, ['#item' => $this->item, '#name' => $phrase]));
        }
    ...
    public function edit($id)
    {
        ...
        return view('admin.system.language_master.form', compact('data'));
    }
    ...
    public function do_edit($id, Request $request)
    {
        ...
        if ($data->save()) {
            // SUCCESS
            return redirect()
                ->route('admin.langmaster.edit', $id)
                ->with('success', lang('Successfully updated #item : #name', $this->translation, ['#item' => $this->item, '#name' => $phrase]));
        }
